...
from Settings.MyUtility import checkin_guild, get_prefix, db_gld, is_number
...
class OWS(commands.Cog):
    ...
    async def load_story(self, channel: discord.TextChannel, person: discord.User, *, page: int = 1):
        stories: list = checkin_guild(channel.guild)["stories"]
    ...
    async def manual(channel: discord.TextChannel):
        emb: discord.Embed = discord.Embed(title="📚 One Word Story | Manual", colour=WHITE)
        emb.add_field(
            name="Goal :",
