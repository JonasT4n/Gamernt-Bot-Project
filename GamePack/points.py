...
class PointSystem(commands.Cog):
    ...
    async def help_coin(self, ctx):
        emb = discord.Embed(title="💲 Currency Help", description="Might Be Usefull in the Future, **Stay Tune!**", colour=discord.Colour(WHITE))
    ...
    async def currency(self, ctx, *args):
        if len(args) == 0:
            ...
            emb = discord.Embed(title=f"{ctx.message.author.name}'s Wallet", description=f"**{info[1]}** 💲", colour=discord.Colour(WHITE))
            await ctx.send(embed=emb)
            ...
            if "@!" in args[0] and len(args) == 1:
                ...
                info = self.db.cursor.fetchone()
                emb = discord.Embed(title=f"{person.name}'s Wallet", description=f"**{info[1]}** 💲", colour=discord.Colour(WHITE))
                await ctx.send(embed=emb)
